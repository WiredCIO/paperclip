...
export const aiConnectionsApi = {
  ...
  list: (companyId: string, agentId?: string) => api.get<{ currentUserId: string; canShareConnections: boolean; connections: AiManagedConnectionSummary[] }>(`/companies/${companyId}/ai-connections${agentId ? `?agentId=${encodeURIComponent(agentId)}` : ""}`),
